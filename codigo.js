...
const eleccionCpu = (numero) => {
   const opciones = ["images/Piedra.png", "images/Tijeras.png", "images/Papel.png"]
   cpu.setAttribute("src", opciones[numero - 1]);

   const opcionCpu = cpu.getAttribute("src");
   const opcionmano = elegido.getAttribute("src")

   if (opcionmano === "images/Piedra.png" && opcionCpu === "images/Tijeras.png" ||
...
const mostrarSiSeTermina = (puntuacionUsuario, puntuacionCpu) => {
   ...
   if (puntuacionCpu == 3) narrador.textContent = `Mala suerte, has perdido la partida, elige una opcion para comenzar otra ronda `

   else if (puntuacionUsuario == 3) narrador.textContent = `Felicidades, has ganado la partida, elige una opcion para comenzar otra ronda `
